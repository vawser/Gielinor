...
public class InterfaceTab
{
    ...
    public void Display()
    {
        if (ImGui.CollapsingHeader("General", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            ImGui.Checkbox($"Multiply UI scale by DPI ({(Runebox.Dpi / 96).ToString("P0", new NumberFormatInfo { PercentPositivePattern = 1, PercentNegativePattern = 1 })})", ref CFG.Current.System_ScaleByDPI);
            if (ImGui.IsItemDeactivatedAfterEdit())
            {
                Runebox.UIScaleChanged?.Invoke(null, EventArgs.Empty);
            }
