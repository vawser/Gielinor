...
public class SceneRenderPipeline
{
    ...
    public unsafe SceneRenderPipeline(RenderScene scene, GraphicsDevice device, int width, int height)
    {
        ...
        SceneParams.Projection = GraphicsUtils.CreatePerspective(device, true, CFG.Current.Viewport_Camera_FOV * (float)Math.PI / 180.0f, width / (float)height, 0.1f, 2000.0f);
