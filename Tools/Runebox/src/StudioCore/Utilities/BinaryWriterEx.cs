...
public class BinaryWriterEx
{
    ...
    {
        WriteChars(text, ReaderEncoding.ASCII, terminate);
    }
    ...
    {
        WriteChars(text, ReaderEncoding.ShiftJIS, terminate);
    }
    ...
    {
        if (BigEndian)
            WriteChars(text, ReaderEncoding.UTF16BE, terminate);
        else
            WriteChars(text, ReaderEncoding.UTF16, terminate);
    }
    ...
    public void WriteFixStr(string text, int size, byte padding = 0)
    {
        ...
        byte[] bytes = ReaderEncoding.ShiftJIS.GetBytes(text + '\0');
    ...
    public void WriteFixStrW(string text, int size, byte padding = 0)
    {
        ...
        byte[] bytes;
        if (BigEndian)
            bytes = ReaderEncoding.UTF16BE.GetBytes(text + '\0');
        else
            bytes = ReaderEncoding.UTF16.GetBytes(text + '\0');
        Array.Copy(bytes, fixstr, Math.Min(size, bytes.Length));
        bw.Write(fixstr);
    }
    #endregion
